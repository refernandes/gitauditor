...
def sandbox_repo_rebase(tmp_path):
    ...
    # Cria 3 commits
    for i in range(3):
        with open(os.path.join(repo_dir, "file.txt"), "a") as f: f.write(f"Line {i}\n")
...
def test_start_interactive_rebase_no_sed(sandbox_repo_rebase, monkeypatch):
    ...
    assert GitService.is_rebasing(sandbox_repo_rebase), "O repositório não entrou em estado de rebase!"
