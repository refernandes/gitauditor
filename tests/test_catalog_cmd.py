...
def mock_db_engine():
    ...
    with (
        patch("gitauditor.commands.catalog_cmd.engine", engine),
...
def test_sync_catalog(mock_db_engine, tmp_git_repo):
    ...
    with (
        patch("gitauditor.commands.catalog_cmd.GitScanner") as mock_scanner_cls,
...
def test_health_dashboard_with_repos(mock_db_engine):
    ...
    with Session(mock_db_engine) as session:
        session.add(
            Repo(name="repo1", path="/path/1", branch="main", status="Synced", canonical_name="a")
        )
        session.add(
            Repo(name="repo2", path="/path/2", branch="main", status="Orphan", canonical_name="b")
        )
        session.commit()
