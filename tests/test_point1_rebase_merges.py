...
def test_reword_preserves_merges(sandbox_repo):
    ...
    res = subprocess.run(["git", "log", "--oneline"], cwd=sandbox_repo, capture_output=True, text=True)
    ...
    res_after = subprocess.run(["git", "log", "--oneline"], cwd=sandbox_repo, capture_output=True, text=True)
