...
class IdentityManager:
    ...
    def list_ssh_keys() -> List[Dict[str, str]]:
        ...
        for pub_key in pub_keys:
            ...
            if os.path.exists(private_key):
                key_type = "Unknown"
                if "rsa" in private_key: key_type = "RSA"
                elif "ed25519" in private_key: key_type = "Ed25519"
                elif "ecdsa" in private_key: key_type = "ECDSA"
    ...
    @staticmethod
    def set_repo_identity(repo_path: str, ssh_key_path: str = None, name: str = None, email: str = None) -> bool:
        """Configura a identidade local e a chave SSH para um repositório específico."""
        try:
            if name:
                subprocess.run(['git', 'config', 'user.name', name], cwd=repo_path, check=True)
            if email:
            ...
            if ssh_key_path:
                ...
                subprocess.run(['git', 'config', 'core.sshCommand', ssh_command], cwd=repo_path, check=True)
            return True
    ...
    async def test_provider_connection(provider: str = "github.com") -> bool:
        ...
        try:
            process = await asyncio.create_subprocess_exec(
                'ssh', '-T', '-o', 'StrictHostKeyChecking=no', '-o', 'ConnectTimeout=5', f'git@{provider}',
