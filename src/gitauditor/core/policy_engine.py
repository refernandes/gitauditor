...
class PolicyEngine:
    ...
    def check_repository(repo_path: str) -> dict[str, Any]:
        ...
        report = {"score": 100, "checks": {}, "warnings": [], "critical": []}
        ...
        if not has_gitignore:
            ...
            report["warnings"].append(
                "Ausência de arquivo .gitignore (risco de lixo no histórico)."
            )
        ...
        if not has_ci:
            report["score"] -= 10
            report["warnings"].append(
                "Ausência de pipeline de CI/CD detectável (.github ou .gitlab-ci)."
            )
        ...
        has_contributing = any(
            os.path.exists(os.path.join(repo_path, f))
            for f in ["CONTRIBUTING.md", "contributing.md"]
        )
        ...
        has_security = any(
            os.path.exists(os.path.join(repo_path, f)) for f in ["SECURITY.md", "security.md"]
        )
        ...
        if not (has_codeowners and has_contributing and has_security):
            report["warnings"].append(
                "Faltam arquivos de Governança (CODEOWNERS, CONTRIBUTING.md ou SECURITY.md)."
            )
        ...
        if is_env_tracked:
            ...
            report["critical"].append(
                "CRÍTICO: O arquivo '.env' está versionado no repositório! Risco de vazamento de credenciais."
            )
