...
def create_worktree(query: str, branch: str):
    ...
    console.print(f"[cyan]Criando worktree para branch '{branch}' em '{dest_path}'...[/cyan]")
    ...
    if res.returncode == 0:
        ...
        AuditLogger.log(
            "worktree_create", "SUCCESS", f"Criada branch {branch}", repo_path=dest_path
        )
    else:
...
def clean_worktrees(
    ...
    force: bool = typer.Option(
        False, "--force", "-f", help="Apaga sem perguntar (requer repositórios limpos)"
    ),
):
    ...
    subprocess.run(["git", "worktree", "prune"], cwd=path, capture_output=True, timeout=15)
    ...
    for wt in worktrees:
        ...
        if not os.path.exists(wt_path):
            ...
            try:
                wt_size = sum(
                    ...
                    for dirpath, _, filenames in os.walk(wt_path)
        ...
        table.add_row(
            os.path.basename(wt_path),
            wt.get("branch", "detached").split("/")[-1],
            status,
            f"{size_mb:.1f} MB",
        )
    ...
    if not to_clean:
        console.print(
            "[yellow]As worktrees encontradas estão SUJAS ou ausentes. Nenhuma ação automática será tomada.[/yellow]"
        )
        return
    ...
    if force or Confirm.ask("\nDeseja apagar essas worktrees e recuperar espaço?"):
        ...
        AuditLogger.log(
            "worktree_clean",
            "SUCCESS",
            f"Limpou {removed} worktrees em {os.path.basename(path)}",
            repo_path=path,
        )
