...
def clean_worktrees(query: str, force: bool = typer.Option(False, "--force", "-f", help="Apaga sem perguntar (requer repositórios limpos)")):
    ...
    res = subprocess.run(["git", "worktree", "list", "--porcelain"], cwd=path, capture_output=True, text=True, timeout=15)
    if res.returncode != 0:
    ...
    main_res = subprocess.run(["git", "rev-parse", "--show-toplevel"], cwd=path, capture_output=True, text=True, timeout=15)
    ...
    for wt in worktrees:
        ...
        if not os.path.exists(wt_path):
            ...
            # Verifica sujeira
            status_res = subprocess.run(["git", "status", "--porcelain"], cwd=wt_path, capture_output=True, text=True, timeout=15)
    ...
    if force or Confirm.ask("\nDeseja apagar essas worktrees e recuperar espaço?"):
        ...
        for c_path, c_branch in to_clean:
            try:
                subprocess.run(["git", "worktree", "remove", "-f", c_path], cwd=path, check=True, capture_output=True, timeout=15)
                removed += 1
