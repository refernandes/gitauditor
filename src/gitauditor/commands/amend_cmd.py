...
def handle_ai_amend(cli):
    ...
    if 0 <= repo_idx < len(cli.repos):
        ...
        while True:
            ...
            commits_table = Table(
                title=f"Histórico (Página {page + 1} de {(len(commits) - 1) // per_page + 1})",
                show_header=True,
        ...
        if mode == "1":
            c_idx = IntPrompt.ask(
                "Digite o índice do commit que deseja alterar (ex: 0 para o mais recente)"
            )
            ...
            n_commits = IntPrompt.ask(
                f"Quantos commits para trás deseja revisar sequencialmente? (Máx {len(commits)})"
            )
        ...
        if "Sem remote" not in details.get("remote", "Sem remote"):
            do_push = Prompt.ask(
                "\n[bold cyan]Deseja enviar (force push) o novo histórico para o GitHub/Remote?[/bold cyan] [S/N]",
                choices=["S", "N", "s", "n"],
                default="N",
            )
            if do_push.upper() == "S":
...
def _process_single_amend(cli, repo_path: str, target_commit: dict, is_batch: bool = False) -> bool:
    ...
    if target_commit["message"].strip() == suggestion.strip():
        console.print(
            "[bold cyan]✅ Commit já passou por esse tratamento (mesma mensagem)![/bold cyan]"
        )
