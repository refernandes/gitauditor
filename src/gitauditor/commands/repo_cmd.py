...
def handle_repo_details(cli):
    ...
    if 0 <= repo_idx < len(cli.repos):
        ...
        while True:
            # Commits
            commits_table = Table(
                title=f"Histórico (Página {page + 1} de {max(1, (len(commits) - 1) // per_page + 1)})",
                show_header=True,
            ...
            if sub_choice == "1":
                dest_dir = Prompt.ask(
                    "Digite o caminho absoluto da pasta de destino (ex: /mnt/arquivos/Projetos)"
                )
                if os.path.exists(dest_dir) and os.path.isdir(dest_dir):
                    final_dest = os.path.join(dest_dir, os.path.basename(repo_path))
                    confirm = Prompt.ask(
                        f"[bold yellow]ATENÇÃO:[/] Confirmar a movimentação de [bold]{repo_path}[/] para [bold]{final_dest}[/]? (S/N)",
                        choices=["S", "N", "s", "n"],
                    )
                    if confirm.lower() == "s":
                        try:
                ...
                confirm = Prompt.ask(
                    f"[bold red]PERIGO:[/] Deseja MESMO deletar [bold]{repo_path}[/] do seu PC? (S/N)",
                    choices=["S", "N", "s", "n"],
                )
                if confirm.lower() == "s":
                    try:
                        ...
                        console.print(
                            f"[bold green]✅ Pasta {repo_path} deletada do sistema![/bold green]"
                        )
                    except Exception as e:
