...
def handle_audit_duplicates(cli):
    ...
    if not duplicate_sets:
        console.print("[green]Nenhum repositório duplicado encontrado apontando para o mesmo origin![/green]")
    ...
    if 0 <= set_idx < len(duplicate_sets):
        ...
        if 0 <= p_idx < len(paths):
            ...
            if choice == "1":
                confirm = Prompt.ask(f"[bold red]ATENÇÃO:[/] Deseja MESMO deletar PERMANENTEMENTE a pasta [bold]{target_path}[/]? (S/N)", choices=["S", "N", "s", "n"])
                if confirm.lower() == 's':
                    try:
                        ...
                        console.print(f"[bold green]✅ Cópia duplicada {target_path} deletada com sucesso![/bold green]")
                    except Exception as e:
                        console.print(f"[bold red]Erro ao deletar:[/] {e}")
                        
            elif choice == "2":
                dest_dir = Prompt.ask("Digite o caminho do diretório destino (ex: /mnt/arquivos/ProjetosUnificados)")
                if os.path.exists(dest_dir) and os.path.isdir(dest_dir):
                    final_dest = os.path.join(dest_dir, os.path.basename(target_path))
                    confirm = Prompt.ask(f"[bold yellow]ATENÇÃO:[/] Mover de [bold]{target_path}[/] para [bold]{final_dest}[/]? (S/N)", choices=["S", "N", "s", "n"])
                    if confirm.lower() == 's':
                        try:
                            ...
                            console.print(f"[bold green]✅ Repositório unificado e movido com sucesso![/bold green]")
                        except Exception as e:
