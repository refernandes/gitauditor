...
def handle_manage_ssh(cli):
    console.print(Panel.fit("[bold magenta]🔑 Gerenciador de Chaves e Identidades SSH[/bold magenta]"))
    ...
    if not keys:
        ...
        confirm = Prompt.ask("\nDeseja testar a conexão SSH das suas chaves atuais com um provedor Git? (S/N)", choices=["S", "N", "s", "n"])
        if confirm.lower() == 's':
            provider = Prompt.ask("Qual provedor?", choices=["github.com", "gitlab.com", "bitbucket.org"], default="github.com")
            with console.status(f"[bold blue]Testando conexão com {provider}...") as status:
                success = asyncio.run(IdentityManager.test_provider_connection(provider))
                if success:
                    console.print(f"[bold green]✅ Autenticação bem-sucedida no {provider}![/bold green]")
                else:
                    console.print(f"[bold red]❌ Falha na autenticação SSH com {provider}. Verifique suas chaves e o ssh-agent.[/bold red]")
